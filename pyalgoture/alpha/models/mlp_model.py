...
class MlpModel(AlphaModel):
    ...
    def fit(
        ...
        for segment in [Segment.TRAIN, Segment.VALID]:
            ...
            df = df.sort(["datetime", "aio_symbol"])
        ...
        for step in range(1, self.n_epochs + 1):
            ...
            if early_stop_count >= self.early_stop_rounds:
                logger.info("Reached early stopping condition, training ended")
                break
    ...
    def _evaluate_step(
        ...
        if loss_val < best_valid_score:
            logger.info(f"\tValidation loss reduced from {best_valid_score:.6f} to {loss_val:.6f}")
    ...
    def detail(self) -> pd.DataFrame | None:  # type: ignore
        ...
        logger.info(f"Hidden layer size: {self.hidden_sizes}")

        # Calculate model total parameter number
        total_params = sum(p.numel() for p in self.model.parameters())
        logger.info(f"Model total parameter number: {total_params:,}")
