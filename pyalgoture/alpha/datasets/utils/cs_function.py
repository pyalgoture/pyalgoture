...
def cs_rank(feature: DataProxy) -> DataProxy:
    ...
    df: pl.DataFrame = feature.df.select(
        pl.col("datetime"), pl.col(feature.key), pl.col("data").rank().over("datetime")
    )
    return DataProxy(df)
...
def cs_mean(feature: DataProxy) -> DataProxy:
    ...
    df: pl.DataFrame = feature.df.select(
        pl.col("datetime"), pl.col(feature.key), pl.col("data").mean().over("datetime")
    )
    return DataProxy(df)
...
def cs_std(feature: DataProxy) -> DataProxy:
    ...
    df: pl.DataFrame = feature.df.select(pl.col("datetime"), pl.col(feature.key), pl.col("data").std().over("datetime"))
    return DataProxy(df)
