...
def ts_delay(feature: DataProxy, window: int) -> DataProxy:
    ...
    df: pl.DataFrame = feature.df.select(
        pl.col("datetime"),
        pl.col(feature.key),
        pl.col("data").shift(window).over(feature.key),
    )
    return DataProxy(df)
...
def ts_min(feature: DataProxy, window: int) -> DataProxy:
    ...
    df: pl.DataFrame = feature.df.select(
        pl.col("datetime"),
        pl.col(feature.key),
        pl.col("data").rolling_min(window, min_samples=1).over(feature.key),
    )
    return DataProxy(df)
...
def ts_max(feature: DataProxy, window: int) -> DataProxy:
    ...
    df: pl.DataFrame = feature.df.select(
        pl.col("datetime"),
        pl.col(feature.key),
        pl.col("data").rolling_max(window, min_samples=1).over(feature.key),
    )
    return DataProxy(df)
...
def ts_argmax(feature: DataProxy, window: int) -> DataProxy:
    ...
    df: pl.DataFrame = feature.df.select(
        pl.col("datetime"),
        pl.col(feature.key),
        pl.col("data").rolling_map(lambda s: cast(int, s.arg_max()) + 1, window).over(feature.key),
    )
    return DataProxy(df)
...
def ts_argmin(feature: DataProxy, window: int) -> DataProxy:
    ...
    df: pl.DataFrame = feature.df.select(
        pl.col("datetime"),
        pl.col(feature.key),
        pl.col("data").rolling_map(lambda s: cast(int, s.arg_min()) + 1, window).over(feature.key),
    )
    return DataProxy(df)
...
def ts_rank(feature: DataProxy, window: int) -> DataProxy:
    ...
    df: pl.DataFrame = feature.df.select(
        pl.col("datetime"),
        pl.col(feature.key),
        pl.col("data").rolling_map(lambda s: stats.percentileofscore(s, s[-1]) / 100, window).over(feature.key),
    )
    return DataProxy(df)
...
def ts_sum(feature: DataProxy, window: int) -> DataProxy:
    ...
    df: pl.DataFrame = feature.df.select(
        pl.col("datetime"),
        pl.col(feature.key),
        pl.col("data").rolling_sum(window).over(feature.key),
    )
    return DataProxy(df)
...
def ts_mean(feature: DataProxy, window: int) -> DataProxy:
    ...
    df: pl.DataFrame = feature.df.select(
        pl.col("datetime"),
        pl.col(feature.key),
        pl.col("data").cast(pl.Float32).rolling_map(lambda s: np.nanmean(s), window, min_samples=1).over(feature.key),
    )
    return DataProxy(df)
...
def ts_std(feature: DataProxy, window: int) -> DataProxy:
    ...
    df: pl.DataFrame = feature.df.select(
        pl.col("datetime"),
        pl.col(feature.key),
        pl.col("data").rolling_map(lambda s: np.nanstd(s, ddof=0), window, min_samples=1).over(feature.key),
    )
    return DataProxy(df)
...
def ts_slope(feature: DataProxy, window: int) -> DataProxy:
    ...
    df: pl.DataFrame = feature.df.select(
        pl.col("datetime"),
        pl.col(feature.key),
        pl.col("data").rolling_map(lambda s: np.polyfit(np.arange(len(s)), s, 1)[0], window).over(feature.key),
    )
    return DataProxy(df)
...
def ts_quantile(feature: DataProxy, window: int, quantile: float) -> DataProxy:
    ...
    df: pl.DataFrame = feature.df.select(
        ...
        pl.col(feature.key),
        pl.col("data")
        .rolling_map(lambda s: s.quantile(quantile=quantile, interpolation="linear"), window)
        .over(feature.key),
    )
    return DataProxy(df)
...
def ts_rsquare(feature: DataProxy, window: int) -> DataProxy:
    ...
    df: pl.DataFrame = feature.df.select(
        pl.col("datetime"),
        pl.col(feature.key),
        pl.col("data").rolling_map(lambda s: rsquare(s), window).over(feature.key),
    )
    return DataProxy(df)
...
def ts_resi(feature: DataProxy, window: int) -> DataProxy:
    ...
    df: pl.DataFrame = feature.df.select(
        pl.col("datetime"),
        pl.col(feature.key),
        pl.col("data").rolling_map(lambda s: resi(s), window).over(feature.key),
    )
    return DataProxy(df)
...
def ts_corr(feature1: DataProxy, feature2: DataProxy, window: int) -> DataProxy:
    ...
    df_merged: pl.DataFrame = feature1.df.join(feature2.df, on=["datetime", feature1.key])

    df: pl.DataFrame = df_merged.select(
        pl.col("datetime"),
        pl.col(feature1.key),
        pl.rolling_corr("data", "data_right", window_size=window, min_samples=1).over(feature1.key).alias("data"),
    )
...
def ts_less(feature1: DataProxy, feature2: DataProxy | float) -> DataProxy:
    ...
    df: pl.DataFrame = df_merged.select(
        pl.col("datetime"),
        pl.col(feature1.key),
        pl.min_horizontal("data", "data_right").over(feature1.key).alias("data"),
    )
...
def ts_greater(feature1: DataProxy, feature2: DataProxy | float) -> DataProxy:
    ...
    df: pl.DataFrame = df_merged.select(
        pl.col("datetime"),
        pl.col(feature1.key),
        pl.max_horizontal("data", "data_right").over(feature1.key).alias("data"),
    )
...
def ts_log(feature: DataProxy) -> DataProxy:
    ...
    df: pl.DataFrame = feature.df.select(
        pl.col("datetime"),
        pl.col(feature.key),
        pl.col("data").log().over(feature.key),
    )
    return DataProxy(df)
...
def ts_abs(feature: DataProxy) -> DataProxy:
    ...
    df: pl.DataFrame = feature.df.select(
        pl.col("datetime"),
        pl.col(feature.key),
        pl.col("data").abs().over(feature.key),
    )
    return DataProxy(df)
