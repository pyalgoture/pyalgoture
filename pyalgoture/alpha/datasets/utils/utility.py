...
class DataProxy:
    ...
    def __init__(self, df: pl.DataFrame, key: str = "aio_symbol") -> None:
        """Constructor"""
    ...
    def result(self, s: pl.Series) -> "DataProxy":
        ...
        result: pl.DataFrame = self.df[["datetime", self.key]]
...
def calculate_by_expression(df: pl.DataFrame, expression: str, key: str = "aio_symbol") -> pl.DataFrame:
    ...
    for column in df.columns:
        ...
        if column in {"datetime", key}:
            continue

        # Cache feature df
        column_df = df[["datetime", key, column]]
        d[column] = DataProxy(column_df, key=key)
...
def calculate_by_polars(df: pl.DataFrame, expression: pl.expr.expr.Expr, key: str = "aio_symbol") -> pl.DataFrame:
    """Execute calculation based on Polars expression"""
    return df.select(["datetime", key, expression.alias("data")])


def to_datetime(arg: datetime | str, tz: tzinfo | None = None) -> datetime:
    ...
    if isinstance(arg, str):
        ...
        # else:
        #     fmt = "%Y%m%d"
        # # return datetime.strptime(arg, fmt) if tz is None else tz.localize(datetime.strptime(arg, fmt))
        # return datetime.strptime(arg, fmt) if tz is None else datetime.strptime(arg, fmt).astimezone(tz)
        return parse(arg) if tz is None else parse(arg).astimezone(tz)
    else:
        return arg
