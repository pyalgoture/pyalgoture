...
class Scheduler:
    ...
    def clear(self, tag: Hashable | None = None) -> None:
        ...
        if tag is None:
            ...
            logger.debug(f'Deleting all jobs tagged "{tag}"')
    ...
    def cancel_job(self, job: "Job") -> None:
        ...
        """
        try:
            logger.debug(f'Cancelling job "{str(job)}"')
...
class Job:
    ...
    * a quantity of `time units` defined by `interval`
    ...
    def __init__(self, interval: int, scheduler: Scheduler | None = None):
        ...
        self.period: datetime.timedelta | None = None
    ...
    def __lt__(self, other: "Job") -> bool:
        ...
        run next.
        """
        if self.next_run is None or other.next_run is None:
            return False
        return bool(self.next_run < other.next_run)

    def __str__(self) -> str:
        if self.job_func is not None and hasattr(self.job_func, "__name__"):
    ...
    def __repr__(self) -> str:
        def format_time(t: datetime.datetime | None) -> str:
            return t.strftime("%Y-%m-%d %H:%M:%S") if t else "[never]"

        def is_repr(j: Any) -> bool:
        ...
        if self.at_time is not None and self.unit is not None:
            return f"Every {self.interval} {self.unit[:-1] if self.interval == 1 else self.unit} at {self.at_time} do {call_repr} {timestats}"
        elif self.unit is not None:
            fmt = f"Every {self.interval} {self.unit[:-1] if self.interval == 1 else self.unit} do {call_repr} {timestats}"
            return fmt
        else:
            return f"Job(interval={self.interval}, unit=None) {timestats}"
    ...
    def at(self, time_str: str) -> "Job":
        ...
        if len(time_values) == 3:
            hour_str, minute_str, second_str = time_values
        elif len(time_values) == 2 and self.unit == "minutes":
            hour_str = "0"
            minute_str = "0"
            _, second_str = time_values
        elif len(time_values) == 2 and self.unit == "hours" and len(time_values[0]):
            hour_str = "0"
            minute_str, second_str = time_values
        else:
            hour_str, minute_str = time_values
            second_str = "0"
    ...
    def run(self) -> Any:
        ...
        if self.next_run is not None and self._is_overdue(self.next_run):
            logger.debug(f"Cancelling job {self}")
            return CancelJob
        return ret
    ...
    def _decode_datetimestr(self, datetime_str: str, formats: list[str]) -> datetime.datetime | None:
        for f in formats:
            try:
...
def next_run() -> datetime.datetime | None:
...
def repeat(job: Job, *args: Any, **kwargs: Any) -> Callable:
    """
    ...
    def _schedule_decorator(decorated_function: Callable) -> Callable:
...
if __name__ == "__main__":
    ...
    def test_func() -> None:
        print(f"test_func {datetime.datetime.now()}")

    def job(message: str = "stuff") -> None:
        print(f"I'm working on: {message} at {datetime.datetime.now()}")
    ...
    schedule.every(10).seconds.do(job, message="first")
    schedule.every(5).to(10).minutes.do(job, message="second")
    schedule.every().minute.do(job, message="third")
    schedule.every().day.at("09:31").do(job, message="fourth")
    schedule.every().minute.at(":00").do(job, message="fifth")
    schedule.every().hour.at(":32").do(job, message="sixth")
    while True:
        schedule.run_pending()
        print(f"next_run:{schedule.next_run} & idle_seconds:{schedule.idle_seconds}")
        time.sleep(1)
