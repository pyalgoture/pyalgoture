...
def calculate_technical_indicators(price_df: pd.DataFrame, period: int = 14) -> dict:
    """
    Calculate RSI, SMA, and Bollinger Bands using TA-Lib.
    Returns a dict with the latest values or defaults if insufficient data.
    """
    try:
        prices = price_df["close"].to_list()
        if not prices or len(prices) < period + 1:
            logger.warning(f"Insufficient data: {len(prices)} prices, need {period + 1}")
            return {"rsi": 50.0, "sma": np.nan, "bb_upper": np.nan, "bb_lower": np.nan, "bb_mid": np.nan}

        # Validate prices
        prices = np.array([float(p) for p in prices if isinstance(p, int | float) and not np.isnan(p)])
        ...
        bb_upper, bb_mid, bb_lower = talib.BBANDS(prices, timeperiod=period, nbdevup=2, nbdevdn=2, matype=0)
        ...
        logger.exception(f"Indicator calculation error: {str(e)}")
        return {
            "rsi": 50.0,
            "sma": prices[-1] if len(prices) else np.nan,
            "bb_upper": prices[-1] if len(prices) else np.nan,
            "bb_lower": prices[-1] if len(prices) else np.nan,
            "bb_mid": prices[-1] if len(prices) else np.nan,
        }


def calculate_sma_crossover(price_df: pd.DataFrame) -> dict:
    ...
    if price_df["sma_fast"].iloc[-1] > price_df["sma_slow"].iloc[-1]:
        signal = "bullish"
        confidence = 70
    elif price_df["sma_fast"].iloc[-1] < price_df["sma_slow"].iloc[-1]:
        signal = "bearish"
        confidence = 70
    return {
        "signal": signal,
        ...
        "metrics": {
            "sma_fast": float(price_df["sma_fast"].iloc[-1]),
            "sma_slow": float(price_df["sma_slow"].iloc[-1]),
        },
    }


def calculate_macd(price_df: pd.DataFrame, fastperiod: int = 12, slowperiod: int = 26, signalperiod: int = 9) -> dict:
    """
    ...
    try:
        prices = price_df["close"].to_list()
        volumes = price_df["volume"].to_list()
        prices = np.array([float(p) for p in prices if isinstance(p, int | float) and not np.isnan(p)])
        volumes = np.array([float(p) for p in volumes if isinstance(p, int | float) and not np.isnan(p)])
        macd, signal, _ = talib.MACD(prices, fastperiod=fastperiod, slowperiod=slowperiod, signalperiod=signalperiod)
        macd_signal = (
            "bullish"
        ...
        return {"signal": macd_signal, "confidence": 70, "metrics": {"macd": macd[-1], "volume_spike": vol_spike}}
    except Exception as e:
        logger.exception(f"Indicator calculation error: {str(e)}")
        return {
            "signal": "neutral",
            "confidence": 50,
            "metrics": {"macd_signal": "unavailable", "volume_spike": "unavailable"},
        }
...
def calculate_volatility_signals(prices_df: pd.DataFrame, interval: str) -> dict:
    """
    ...
    hist_vol = returns.rolling(21).std() * math.sqrt(365 * 24 if "h" in interval else 365)
...
def normalize_pandas(
    obj: pd.Series | pd.DataFrame | dict | list | tuple,
) -> pd.Series | pd.DataFrame | dict | list | tuple:
