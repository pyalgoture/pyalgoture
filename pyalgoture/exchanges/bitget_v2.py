...
BITGET_WS_PUBLIC_HOST = "wss://ws.bitget.com/v2/ws/public"
BITGET_WS_PRIVATE_HOST = "wss://ws.bitget.com/v2/ws/private"

BITGET_WS_SPOT_HOST = ""
BITGET_WS_CONTRACT_HOST = ""
...
class BitgetClient(RestClient):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.DATA_TYPE == "linear":
            ...
            if self.is_testnet:
                self.product_type = "SCOIN-FUTURES"
            else:
                self.product_type = "COIN-FUTURES"
    ...
    def _get_margin_coin(self, symbol):
        margin_coin = None
        if self.DATA_TYPE == "linear":
            if self.product_type in ["USDC-FUTURES", "SUSDC-FUTURES"]:
    ...
    def _process_response(self, response: Response) -> dict:
        ...
        try:
            ...
            print(f"[BITGET] in _process_response, something went wrong when parsing data. Raw data:{response.text}")
            ...
            if not response.ok or (response.ok and not data["data"] and data["code"] != "00000"):
                payload_data = models.CommonDataSchema(status_code=response.status_code, msg=dict(data))
                if not data.get("msg"):
                    print(f"[BITGET] request without msg msg. Raw data::{response.text}")
    ...
    def _regular_order_payload(self, common_response: models.CommonResponseSchema) -> dict:
        if isinstance(common_response.data, list):
            ...
            for msg in common_response.data:
                ...
                result = data.from_bitget_v2_to_form(msg, datatype=self.DATA_TYPE)
                payload.append(result)
        elif isinstance(common_response.data, dict):
            payload = models.OrderSchema()
            payload = payload.from_bitget_v2_to_form(common_response.data, datatype=self.DATA_TYPE)
        else:
    ...
    def _regular_historical_prices_payload(
        ...
        if isinstance(common_response.data, list):
            ...
            for msg in common_response.data:
                ...
                result = data.from_bitget_v2_to_form(msg, extra, datatype=self.DATA_TYPE)
                payload.append(result)
        elif isinstance(common_response.data, dict):
            if self.product_type:
                common_response.data = key_data(common_response.data)
            payload = models.HistoryOHLCSchema()
            payload = payload.from_bitget_v2_to_form(common_response.data, extra, datatype=self.DATA_TYPE)
        else:
            common_response.msg = "payload error in order"
            Exception("payload error in order")
    ...
    def _regular_ticker_payload(self, common_response: models.CommonResponseSchema, symbol: str) -> dict:
        payload = list()
        if isinstance(common_response.data, list):
            for msg in common_response.data:
                data = models.TickerSchema()
                payload = data.from_bitget_v2_to_form(msg, symbol=symbol, datatype=self.DATA_TYPE)
                # payload.append(result)
        elif isinstance(common_response.data, dict):
            payload = models.TickerSchema()
            payload = payload.from_bitget_v2_to_form(common_response.data, symbol=symbol, datatype=self.DATA_TYPE)
        else:
    ...
    def _regular_transfer_payload(
        ...
        if isinstance(common_response.data, list):
            ...
            for msg in common_response.data:
                ...
                result = data.from_bitget_v2_to_form(
                    msg,
            ...
            payload = payload.from_bitget_v2_to_form(
    ...
    def _regular_account_payload(self, common_response: models.CommonResponseSchema, account_type: str) -> dict:
        if isinstance(common_response.data, list):
            ...
            for msg in common_response.data:
                ...
                result = data.from_bitget_v2_to_form(msg, datatype=self.DATA_TYPE, account_type=account_type)
                if result:
                    # payload.append(result)
                    payload[result["symbol"]] = result
        elif isinstance(common_response.data, dict):
    ...
    def query_symbols(self, symbol: str = None) -> dict:
        ...
        symbols = self._get("/api/v2/spot/public/symbols", payload)
    ...
    def query_history(
            ...
            while interval_timestamp > interval:
                ...
                if self.product_type:
                    historical_prices_datalist = [
                        list(item) for item in set(tuple(x.items()) for x in historical_prices_datalist)
                    ]
                    historical_prices_datalist.sort(key=lambda k: k[0][0])
                else:
                    historical_prices_datalist = [
                        dict(item) for item in set(tuple(x.items()) for x in historical_prices_datalist)
                    ]
        ...
        if self.product_type:
            ...
            if start:
                ...
                payload["startTime"] = int(datetime.timestamp(start) * 1000)  # NOTE: start
            ...
            payload = {
                "symbol": symbol.upper(),
                "granularity": KBAR_SPOT_INTERVAL[interval],
                "limit": limit,
            }
            end = end.replace(tzinfo=None)
            end = UTC_TZ.localize(end)
            payload["endTime"] = int(datetime.timestamp(end) * 1000)  # NOTE: end
            url = "/api/v2/spot/market/history-candles"
    ...
    def query_last_price(
        ...
        if self.product_type:
            payload = {
                ...
                "granularity": KBAR_CONTRACT_INTERVAL[interval],
                "limit": limit,
            }  #  "startTime": int(time.time() * 1000), "endTime": int(time.time() * 1000)
            last_historical_prices = self._get("/api/v2/mix/market/candles", payload)
        else:
            payload = {
                "symbol": symbol,
                "granularity": KBAR_SPOT_INTERVAL[interval],
            ...
            last_historical_prices = self._get("/api/v2/spot/market/candles", payload)
    ...
    def query_prices(self, symbol: str = None) -> dict:
        ...
        spots = self._get(
            url if self.product_type else "/api/v2/spot/market/tickers",
            params=remove_none(payload),
        )
    ...
    def query_permission(self) -> dict:
        """ """
        api_info = self._get("/api/v2/spot/account/info")
        # print(f">>>> api_info:{api_info}")
        if api_info.success:
            payload = models.PermissionSchema()  # .model_construct(api_info.data)
            api_info.data = payload.from_bitget_v2_to_form(api_info.data)
        return api_info
    ...
    def query_account(self) -> dict:
        ...
        if self.product_type:
            payload["productType"] = self.product_type
        balance = self._get(
            "/api/v2/mix/account/accounts" if self.product_type else "/api/v2/spot/account/assets",
            payload,
        )
    ...
    def send_order(
        ...
        **kwargs,
        ...
        """ """
        if position_side not in ["long", "short", "net"]:
            return models.CommonResponseSchema(
                success=False,
                error=True,
                data={},
        ...
        if self.product_type:
            payload = {
                ...
                "marginMode": "isolated" if is_isolated else "crossed",
                # "posMode":"one_way_mode"
            }
            if position_side and position_side.lower() != "net":
                # Only required in hedge mode. open and close position.
                ...
                payload["side"] = "buy" if position_side.lower() == "long" else "sell"
                payload["tradeSide"] = "close" if reduce_only else "open"
        ...
        if order_type.upper() == "LIMIT":
            ...
            if self.DATA_TYPE.lower() == "spot":
                ...
                print(self.query_prices(symbol), "????")
                last_price = self.query_prices(symbol)["data"]["price"]
                payload["size"] = round(last_price * quantity, 4)
                print(f"payload: {payload} ????in bitget spot send_order???? last_price: {last_price}")
        ...
        if orders_response.success:
            if orders_response.data:
                ...
                orders_response.data = result.from_bitget_v2_to_form(
    ...
    def cancel_order(self, order_id: str, symbol: str) -> dict:
        ...
        # else:
        #     if not symbol:
        #         return models.CommonResponseSchema(success=False, error=True, data={}, msg=f"symbol is missed")

        # return self._post("/api/spot/v1/trade/cancel-order-v2", payload)
        return self._post(
            "/api/v2/mix/order/cancel-order" if self.product_type else "/api/spot/v1/trade/cancel-order",
            payload,
        )
    ...
    def query_order_status(self, order_id: str, symbol: str) -> dict:
        ...
        order_status = self._get(
            "/api/v2/mix/order/detail" if self.product_type else "/api/v2/spot/trade/orderInfo",
            payload,
        )
    ...
    def query_open_orders(self, symbol: str = None) -> dict:
        ...
        order_status = self._get(
            "/api/v2/mix/order/orders-pending" if self.product_type else "/api/v2/spot/trade/unfilled-orders",
            payload,
        )

        # print(f"query_open_orders: {order_status} | payload:{payload}")
        if order_status.success:
            if order_status.data and order_status.data["entrustedList"]:
                order_status.data = order_status.data["entrustedList"]
                return self._regular_order_payload(order_status)
            else:
                order_status.msg = "query order is empty"
                order_status.data = []
    ...
    def query_trades(self, symbol: str, start: datetime = None, end: datetime = None, limit=100) -> dict:
        """ """
        payload = {
            "limit": limit,
            "symbol": symbol,
        ...
        #     payload["symbol"] = symbol
        # else:
        #     return models.CommonResponseSchema(success=False, error=True, data={}, msg=f"Please specify the symbol.")
        ...
        trades = self._get(
            "/api/v2/mix/order/fill-history" if self.product_type else "/api/v2/spot/trade/fills",
            remove_none(payload),
        )
        # print(f"trades: {trades} | payload:{payload}")

        if trades.success:
            if trades.data and trades.data["fillList"]:
                trades.data = trades.data["fillList"]
    ...
    def query_position(self, symbol: str = None, force_return: bool = False, **kwargs) -> dict:
        """ """
        ...
        if symbol:
            payload["symbol"] = symbol.upper()
            payload["marginCoin"] = self._get_margin_coin(symbol)
            url = "/api/v2/mix/position/single-position"
        else:
            payload["marginCoin"] = "USDT"
            url = "/api/v2/mix/position/all-position"

        position = self._get(url, payload)
    ...
    def set_position_mode(self, mode: str) -> dict:
        ...
        payload = {
            "productType": self.product_type,
            "posMode": "one_way_mode" if mode == "oneway" else "hedge_mode",
        }

        mode_payload = self._post("/api/v2/mix/account/set-position-mode", remove_none(payload))
    ...
    def set_leverage(self, leverage: float, symbol: str, is_isolated: bool = True) -> dict:
        """ """
        if not self.product_type:
            return models.CommonResponseSchema(
                success=False,
        ...
        marginmode_payload = self._post("/api/v2/mix/account/set-margin-mode", remove_none(m_payload))
        ...
        if is_isolated:
            for s in ["long", "short"]:
                payload["holdSide"] = s
                leverage_payload = self._post("/api/v2/mix/account/set-leverage", remove_none(payload))
...
class BitgetDataWebsocket(WebsocketClient):
    ...
    def on_connected(self) -> None:
        ...
        print(f"===> debug Bitget {self.DATA_TYPE} Market data on_connected - self.ticks: {self.ticks}")
    ...
    def on_disconnected(self) -> None:
        print(f"Bitget {self.DATA_TYPE} market data Websocket disconnected, now try to connect @ {datetime.now()}")
    ...
    def subscribe(
        ...
        for symbol in symbols:
            ...
            # tick data dict
            asset_type = models.bitget_asset_type(symbol=symbol, datatype=self.DATA_TYPE)
            tick = {
                "code": symbol,
                "exchange": EXCHANGE,
                "channel": channel,
            ...
            self.ticks[key] = tick
            req: dict = self.sub_stream(symbol=instid, channel=channel, interval=interval)
            self.send_packet(req)
    ...
    def on_packet(self, packet: dict) -> None:
        ...
        if "candle" in channel:
            ...
            tick["prev_open_24h"] = float(data["open24h"]) if "open24h" in data else None
            ...
            tick["price_change_pct"] = float(data["priceChangePercent"]) if "priceChangePercent" in data else None
            tick["prev_close_24h"] = (
                (tick["last_price"] / (1 + tick["price_change_pct"])) if tick["price_change_pct"] else None
            )
            tick["price_change"] = (tick["last_price"] - tick["prev_close_24h"]) if tick["price_change_pct"] else None
...
class BitgetTradeWebsocket(BitgetDataWebsocket):
    ...
    def connect(self, key: str, secret: str, passphrase: str, is_testnet: bool = None):
        ...
        if not self._logged_in:
            if self.debug:
                print(f">>> in [connect] send_auth_packet [{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}]")
            self._login()
        return True
    ...
    def _login(self):
        ...
        sign_message = BitgetClient.pre_hash(timestamp=str(ts), method="GET", request_path="/user/verify", body=None)
    ...
    def on_disconnected(self) -> None:
        print(f"Bitget user trade data Websocket disconnected, now try to connect @ {datetime.now()}")
        self._logged_in = False
        if self.on_disconnected_callback:
            self.on_disconnected_callback()
    ...
    def on_connected(self) -> None:
        ...
        if not self._logged_in:
            if self.debug:
                print(f">>> in [on_connected] send_auth_packet [{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}]")
            self._login()
    ...
    def _subscribe(self):
        ...
        if self.debug:
            print(f">>> in [on_connected] send_sub_packet [{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}]")
        self.send_packet(req)
        if self.on_connected_callback:
            self.on_connected_callback()
    ...
    def sub_stream(self, channel="orders"):
        ...
        if isinstance(channel, list):
            ...
            sub["args"] = [
                {
                    "channel": MAPPING_CHANNEL[channel] if channel != "trades" else "orders",
                    "instId": "default",
                    "instType": self.product_type.upper(),
                }
            ]
    ...
    def on_account(self, packet: dict) -> None:
        ...
        for data in packet["data"]:
            ...
            account = account.from_bitget_v2_to_form(data, datatype=self.DATA_TYPE, account_type=account_type)
    ...
    def on_order(self, packet: dict) -> None:
        ...
        for data in datas:
            ...
            order = order.from_bitget_v2_to_form(data, datatype=self.DATA_TYPE)
    ...
    def on_trade(self, data: dict) -> None:
        ...
        trade = trade.from_bitget_v2_to_form(data, datatype=self.DATA_TYPE)
    ...
    def on_position(self, packet: dict) -> None:
        ...
        for data in datas:
            ...
            position = position.from_bitget_v2_to_form(data, datatype=self.DATA_TYPE)
