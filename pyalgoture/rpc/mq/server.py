...
Server      Client       Usage
publish     subscibe >>  server.publish(topic, data) & client.subscribe_topic(topic)+callback(data)  [topic='' will publish to all clients / receive all from server]
response    request  >>  server.register(func_name, client_id) & client.XXXfunc()                    [client_id='' all the client can call]
"""
...
class RpcServer:
    ...
    def publish(self, topic: str, data: Any) -> None:
        ...
        with self._lock:
            ...
            self._socket_pub.send_multipart([topic.encode("utf-8"), pickle.dumps(data)])

    def register(self, func: Callable, client_id: str = "") -> None:
        """
        Register function
        """
        self._functions[f"{client_id}{func.__name__}"] = func

    def check_heartbeat(self, msg: str = "") -> None:
        """
