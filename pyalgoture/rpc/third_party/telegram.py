...
from ..rpc_types import RPCEntryMsg, RPCExitMsg, RPCOrderMsg, RPCSendMsg
...
def round_value(value: float, decimals: int = 8) -> str:
    """Round value to specified decimals"""
    return f"{value:.{decimals}f}"


def fmt_coin(value: float, currency: str, show_currency: bool = True) -> str:
    """Format coin value"""
    if show_currency:
...
def fmt_coin2(value: float, currency: str) -> str:
    """Format coin value with currency"""
    return f"{value:.8f} {currency}"


def dt_humanize_delta(dt) -> str:
    """Simple humanized time delta"""
    if dt is None:
        return "N/A"
    return str(dt)


def dt_from_ts(ts: float):
...
# Constants
DUST_PER_COIN = {"USDT": 0.01, "BTC": 0.00001, "ETH": 0.0001}
...
def authorized_only(command_handler: Callable[..., Coroutine[Any, Any, None]]):
    ...
    async def wrapper(self, *args, **kwargs) -> None:
        ...
        message: Message = update.message if update.callback_query is None else update.callback_query.message
...
class Telegram(RPCHandler):
    ...
    def _init_keyboard(self) -> None:
        ...
        if cust_keyboard:
            ...
            invalid_keys = [b for b in chain.from_iterable(cust_keyboard) if not re.match(combined, b)]
            if len(invalid_keys):
                err_msg = (
    ...
    def _init(self) -> None:
        ...
        try:
            ...
            handles = [
                ...
                CommandHandler("stopentry", self._stopentry),
            ...
            logger.info(
                f"rpc.telegram is listening for following commands: {[[x for x in sorted(h.commands)] for h in handles]}"
            )
            ...
            self._initialized = False
            # Don't raise the exception to prevent the entire application from crashing
            # The bot will simply not be available
    ...
    async def _startup_telegram(self) -> None:
        ...
        while attempt < retries:
            try:
                ...
                logger.error(f"Error starting Telegram bot (attempt {attempt + 1}/{retries}): {ex}")
                attempt += 1
                if attempt == retries:
            ...
            try:
                ...
                while True:
                    try:
                        ...
                        if not self._app.updater.running or self._shutdown_requested:
                            break
                    except (asyncio.CancelledError, RuntimeError):
                        # Handle shutdown gracefully
                        logger.info("Telegram polling interrupted during shutdown")
                        break
                    except Exception as e:
    ...
    async def _cleanup_telegram(self) -> None:
        try:
            ...
            logger.error(f"Error stopping Telegram updater: {e}")
    ...
    def cleanup(self) -> None:
        ...
        # Signal shutdown to polling loop
        ...
        if hasattr(self, "_thread") and self._thread and self._thread.is_alive():
            self._thread.join(timeout=10)  # Give thread 10 seconds to cleanup
            if self._thread.is_alive():
                logger.warning("Telegram thread did not stop gracefully within timeout")

        logger.info("Telegram bot cleanup completed.")

    def is_ready(self) -> bool:
        """Check if Telegram bot is ready to receive messages"""
        return self._initialized and hasattr(self, "_loop") and self._loop is not None
        ...
        if candle_val != "off":
            if candle_val == "ohlc":
                ...
                if candle is not None:
                    return f"*Candle OHLC*: `{candle['open']}, {candle['high']}, {candle['low']}, {candle['close']}`\n"
    ...
    def _format_entry_msg(self, msg: RPCEntryMsg) -> str:
        ...
        message = f"{emoji} *{self._exchange_from_msg(msg)}:* {wording} (#{msg['trade_id']})\n*Pair:* `{msg['pair']}`\n"
    ...
    def _format_exit_msg(self, msg: RPCExitMsg) -> str:
        ...
        leverage_text = f" ({msg['leverage']:.3g}x)" if msg.get("leverage") and msg.get("leverage", 1.0) != 1.0 else ""

        profit_fiat_extra = self.__format_profit_fiat(msg, "profit_amount")

        profit_extra = f" ({msg['gain']}: {fmt_coin(msg['profit_amount'], msg['quote_currency'])}{profit_fiat_extra})"
        ...
        if is_sub_trade or is_final_exit:
            ...
            if is_final_exit:
                ...
                if msg["cumulative_profit"]:
                    cp_extra = (
                        f"*Cumulative Profit:* `{fmt_coin(msg['cumulative_profit'], msg['stake_currency'])}{cp_fiat}`\n"
                    )
    ...
    def __format_profit_fiat(
        ...
        if self._rpc._fiat_converter and (fiat_currency := msg.get("fiat_currency")):
            profit_fiat = self._rpc._fiat_converter.convert_amount(msg[key], msg["stake_currency"], fiat_currency)
    ...
    async def _status_msg(self, update: Update, context: CallbackContext) -> None:
        ...
        for r in results:
            ...
            r["num_exits"] = len(
                [o for o in r["orders"] if not o["ft_is_entry"] and not o["ft_order_side"] == "stoploss"]
            )
            r["exit_reason"] = r.get("exit_reason", "")
            r["stake_amount_r"] = fmt_coin(r["stake_amount"], r["quote_currency"])
            r["max_stake_amount_r"] = fmt_coin(r["max_stake_amount"] or r["stake_amount"], r["quote_currency"])
            ...
            if r["is_open"]:
                ...
                lines.append(" ")
                if r["stop_loss_abs"] != r["initial_stop_loss_abs"] and r["initial_stop_loss_ratio"] is not None:
                    # Adding initial stoploss only if it is different from stoploss
                    lines.append("*Initial Stoploss:* `{initial_stop_loss_abs:.8f}` `({initial_stop_loss_ratio:.2%})`")
                ...
                if r.get("open_orders"):
                    lines.append(
                        "*Open Order:* `{open_orders}`" + ("- `{exit_order_status}`" if r["exit_order_status"] else "")
                    )
    ...
    def _format_profit_message(
        ...
        no_trades_msg = f"No{direction_label} trades yet.\n*Bot started:* `{stats['bot_start_date']}`"
        ...
        if stats["closed_trade_count"] > 0:
            fiat_closed_trades = f"∙ `{fmt_coin(profit_closed_fiat, fiat_disp_cur)}`\n" if fiat_disp_cur else ""
            markdown_msg = (
                f"{closed_roi_label}\n"
    ...
    async def _profit_handler(
        ...
        markdown_msg = self._format_profit_message(stats, stake_cur, fiat_disp_cur, timescale, direction)
    ...
    async def _stats(self, update: Update, context: CallbackContext) -> None:
        ...
        duration_msg = tabulate(
            [
                [
                    "Wins",
                    (str(timedelta(seconds=durations["wins"])) if durations["wins"] is not None else "N/A"),
                ],
                [
                    "Losses",
                    (str(timedelta(seconds=durations["losses"])) if durations["losses"] is not None else "N/A"),
                ],
            ],
    ...
    async def _balance(self, update: Update, context: CallbackContext) -> None:
        ...
        for curr in result["currencies"]:
            ...
            if (curr["is_position"] or curr["est_stake"] > balance_dust_level) and (
                ...
                if curr["is_position"]:
                    ...
                    est_stake = fmt_coin(curr["est_stake" if full_result else "est_stake_bot"], curr["stake"], False)
        ...
        total_stake = fmt_coin(result["total" if full_result else "total_bot"], result["stake"], False)
        fiat_estimated_value = f"\t`{result['symbol']}: {value}`{fiat_val}\n" if result["symbol"] else ""
        output += (
    ...
    async def _stopentry(self, update: Update, context: CallbackContext) -> None:
        """
        Handler for /stopentry.
        Sets max_open_trades to 0
        :param bot: telegram bot
        :param update: message update
        :return: None
        """
    ...
    async def _force_exit(self, update: Update, context: CallbackContext) -> None:
        ...
        if context.args:
            ...
            fiat_currency = self._config.get("fiat_display_currency", "")
            try:
                statlist, _, _, _ = self._rpc._rpc_status_table(self._config["stake_currency"], fiat_currency)
            except RPCException:
            ...
            trade_buttons = [
                InlineKeyboardButton(text=trade[1], callback_data=f"force_exit__{trade[0]}") for trade in trades
            ]
            buttons_aligned = self._layout_inline_keyboard(trade_buttons, cols=1)

            buttons_aligned.append([InlineKeyboardButton(text="Cancel", callback_data="force_exit__cancel")])
    ...
    @staticmethod
    def _layout_inline_keyboard(buttons: list[InlineKeyboardButton], cols=3) -> list[list[InlineKeyboardButton]]:
    ...
    async def _help(self, update: Update, context: CallbackContext) -> None:
        ...
        message = (
            ...
            "*/health:* `Show latest process timestamp` \n"
            "_Statistics_\n"
            "------------\n"
            ...
            "*/stats:* `Shows Wins / losses by exit reason as well as "
    ...
    async def _update_msg(
        ...
        try:
            ...
            if "not modified" in e.message.lower():
                ...
                logger.warning(f"TelegramError: {e.message}")
        except TelegramError as telegram_err:
            logger.warning(f"TelegramError: {telegram_err.message}! Giving up on that message.")
    ...
    async def _send_msg(
        ...
        try:
            try:
                ...
                logger.warning(f"Telegram NetworkError: {network_err.message}! Trying one more time.")
                await self._app.bot.send_message(
                    self._config["telegram"]["chat_id"],
                    text=msg,
                    parse_mode=parse_mode,
    ...
    async def _tg_info(self, update: Update, context: CallbackContext) -> None:
        ...
        user_id = update.effective_user.id if topic_id is not None and update.effective_user else None
